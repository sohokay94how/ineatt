...
DialogWorkFrame::DialogWorkFrame(bool showSearchBae, bool showAppBar, QWidget *parent)
    : EbDialogBase(parent)
    , EbWorkList(this)
    , ui(new Ui::DialogWorkFrame)
    ...
    , m_showedLeft(true)
{
    ...
    if (m_showSearchBar) {
        ...
        IconHelper::Instance()->SetIcon(ui->pushButtonAdd,QChar(0xf067),10);
...
DialogWorkFrame::~DialogWorkFrame()
{
...
void DialogWorkFrame::updateLocaleInfo()
{
...
void DialogWorkFrame::setSubUnreadMsg( mycp::bigint subId, size_t unreadMsgCount )
{
    if (m_widgetAppBar!=0) {
...
void DialogWorkFrame::addSubUnreadMsg( mycp::bigint subId )
{
    if (m_widgetAppBar!=0) {
...
void DialogWorkFrame::addUrl(bool bSaveBrowseTitle, const QString &sAppUrl, const QString &sPostData, int nInsertOffset)
{
...
void DialogWorkFrame::addUrl(bool bSaveBrowseTitle, const QString &sAppUrl, const QString &sPostData, const EB_SubscribeFuncInfo &pFuncInfo, bool nOpenNewClose, int nInsertOffset)
{
...
void DialogWorkFrame::onClickedButtonTop()
{
...
void DialogWorkFrame::onUrlChanged(const EbWidgetWorkView *, const QUrl &url)
{
    if (m_widgetSearchBar==0) {
        return;
    }
...
void DialogWorkFrame::onLoadStateChange(const EbWidgetWorkView *view, bool loadFinished, bool canGoBack, bool canGoForward)
{
    if (m_widgetSearchBar==0) {
        return;
    }
...
void DialogWorkFrame::onOpenLinkInNewTab(const EbWidgetWorkView *view, const QUrl &linkUrl)
{
...
void DialogWorkFrame::onWindowCloseRequested(const EbWidgetWorkView *view)
{
    /// 必须使用线程关闭
...
void DialogWorkFrame::onClickedBack()
{
    EbWorkList::back();
}

void DialogWorkFrame::onClickedForward()
{
    EbWorkList::forward();
}

void DialogWorkFrame::onClickedRefresh()
{
    EbWorkList::refreshOrStop();
}

void DialogWorkFrame::onClickedStop()
{
    EbWorkList::refreshOrStop();
}

void DialogWorkFrame::onSearchKeyPressEnter(const QString &text)
{
    addUrl(true,text,"");
}

void DialogWorkFrame::onSaveUrlHistory()
{
    EbWorkList::saveUrlHistory();
}

void DialogWorkFrame::onClickedSearchResultUrl(const QString &url)
{
    addUrl(true,url,"");
}

void DialogWorkFrame::onClickedPushButtonShowHideLeft()
{
...
void DialogWorkFrame::onClickedButtonAdd()
{
    addUrl(false,"about:blank","");
}

void DialogWorkFrame::onClickedSubApp(const EB_SubscribeFuncInfo & subFuncInfo)
{
...
void DialogWorkFrame::updateShowHideIcon()
{
    if (m_showedLeft) {
        IconHelper::Instance()->SetIcon(ui->pushButtonShowHideLeft,QChar(0xf060),9);
    }
    else {
        IconHelper::Instance()->SetIcon(ui->pushButtonShowHideLeft,QChar(0xf061),9);
    }
...
void DialogWorkFrame::resizeEvent(QResizeEvent * e)
{
    moveSize();
    EbDialogBase::resizeEvent(e);
}

bool DialogWorkFrame::onBeforeClickedPushButtonSysMin()
{
...
bool DialogWorkFrame::onBeforeClickedPushButtonSysMax()
{
    DialogFrameList* parent = (DialogFrameList*)this->parentWidget();
...
void DialogWorkFrame::reject()
{
    DialogFrameList* parent = (DialogFrameList*)this->parentWidget();
    parent->reject();
}

void DialogWorkFrame::timerEvent(QTimerEvent *event)
{
...
//void DialogWorkFrame::closeEvent(QCloseEvent *event)
//{
//    EbWorkList::clear();
//    EbDialogBase::closeEvent(event);
//}

void DialogWorkFrame::onItemSizeChange(const EbWorkItem::pointer & fromWorkItem,int size, int right)
{
    if (right==0) {
