...
#include "ui_dialogframelist.h"
#include "iconhelper.h"
#include "dialogchatbase.h"
#include "dialogworkframe.h"

DialogFrameList::DialogFrameList(QWidget *parent)
    : EbDialogBase(parent)
    , EbFrameList(this)
    , ui(new Ui::DialogFrameList)
    , m_showedLeft(true)
...
//    DialogChatBase::pointer dialogChatBase = DialogChatBase::create(pCallInfo,this);
...
DialogFrameList::~DialogFrameList()
{
...
void DialogFrameList::updateLocaleInfo()
{
    if (m_showedLeft) {
...
void DialogFrameList::onClickedPushButtonShowWorkFrame()
{
    showWorkFrame();
}

void DialogFrameList::onClickedPushButtonShowHideLeft()
{
...
void DialogFrameList::onClickedButtonLeft(bool /*checked*/)
{
...
void DialogFrameList::onClickedCloseFromDialogChatBase()
{
    const DialogChatBase* chatBase = dynamic_cast<DialogChatBase*>( sender() );
...
void DialogFrameList::onCheckedFrameitem(const EbFrameItem * frameItem)
{
    this->setWindowTitle( frameItem->itemText() );
}

void DialogFrameList::onWorkItemSizeChange(bool ,int size)
{
    if (size<=0) {
...
void DialogFrameList::contextMenuEvent(QContextMenuEvent *e)
{
    m_menuContext->exec( e->globalPos() );
    EbDialogBase::contextMenuEvent(e);
}

void DialogFrameList::moveEvent(QMoveEvent *event)
{
//    EbFrameList::onMoveEvent();
    EbDialogBase::moveEvent(event);
}

void DialogFrameList::resizeEvent(QResizeEvent *e)
{
    updateSize();
...
//bool DialogFrameList::event(QEvent *e)
//{
...
void DialogFrameList::checkOneSecond()
{
...
void DialogFrameList::timerEvent(QTimerEvent *event)
{
...
//void DialogFrameList::customEvent(QEvent *e)
//{
//    const QEvent::Type eventType = e->type();
//    switch ((EB_COMMAND_ID)eventType) {
//    case EB_COMMAND_SYS_MIN:
...
void DialogFrameList::updateShowHideIcon()
{
...
void DialogFrameList::updateSize()
{
    const EB_UI_STYLE_TYPE nDefaultUIStyleType = theApp->defaultUIStyleType();
