...
IconHelper* IconHelper::Instance(void)
{
    static QMutex mutex;
    if (_instance == NULL) {
...
void IconHelper::SetIcon( qint64 subId, QLabel* ctl, int size ) {
    SetIcon( ctl,GetIcon(subId),size );
}

void IconHelper::SetIcon(qint64 subId, QPushButton *ctl, int size)
{
    SetIcon( ctl,GetIcon(subId),size );
}

QChar IconHelper::GetIcon(qint64 subId) const
{
    switch (subId) {
