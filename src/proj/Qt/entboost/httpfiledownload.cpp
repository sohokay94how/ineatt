...
HttpFileDownload::HttpFileDownload(QObject *parent) : QObject(parent)
...
HttpFileDownload::~HttpFileDownload(void)
{
    if (m_file!=NULL) {
        m_file->close();
        delete m_file;
...
void HttpFileDownload::doDownloadHttpFile(void)
{
...
void HttpFileDownload::downloadHttpFile( const QString& url, const QString& saveToFilePath, const QString& oldLastModifited, int nWaitSeconds )
{
    m_downloadFinished = false;
...
void HttpFileDownload::replyReadyRead(void)
{
    if (m_reply==NULL) {
        return;
    }
...
void HttpFileDownload::replyFinished(void)
{
    //获取响应的信息，状态码为200表示正常
...
void HttpFileDownload::replyError(QNetworkReply::NetworkError errorCode)
{
    if (m_reply==NULL) {
        return;
    }
