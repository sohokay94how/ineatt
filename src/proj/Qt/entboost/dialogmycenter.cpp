...
#include "ui_dialogmycenter.h"
#include "ebwidgetaccountinfo.h"
#include "ebwidgetmysetting.h"
#include "ebwidgetmodifypassword.h"
#include "iconhelper.h"

DialogMyCenter::DialogMyCenter(QWidget *parent) :
    EbDialogBase(parent),
    ui(new Ui::DialogMyCenter)
  ...
  , m_widgetModifyPassword(0)
{
    ...
    IconHelper::Instance()->SetIcon(m_labelAccountInfoIcon,QChar(0xf2c3),10);
...
//    connect( ui->pushButtonMySetting,SIGNAL(clicked()),this,SLOT(onClickedPushButtonMySetting()) );
    ...
    IconHelper::Instance()->SetIcon(m_labelModifyPasswordIcon,QChar(0xf023),10);
...
DialogMyCenter::~DialogMyCenter()
{
    delete ui;
}

void DialogMyCenter::updateLocaleInfo()
{
...
void DialogMyCenter::onAreaInfo(const EB_AreaInfo *pAreaInfo, int nParameter)
{
    if (m_widgetAccountInfo!=0)
        m_widgetAccountInfo->onAreaInfo(pAreaInfo,nParameter);
}

void DialogMyCenter::onClickedPushButtonAccountInfo()
{
    if (m_widgetAccountInfo==0) {
...
void DialogMyCenter::onClickedPushButtonMySetting()
{
//    udpateClickedPushButton( sender() );
...
void DialogMyCenter::onClickedPushButtonModifyPassword()
{
...
void DialogMyCenter::udpateClickedPushButton(const QObject *sender)
{
