...
#include "ui_dialogconnectsetting.h"
#include "ebclientapp.h"

DialogConnectSetting::DialogConnectSetting(QWidget *parent) :
    EbDialogBase(parent),
    ui(new Ui::DialogConnectSetting)
{
    ui->setupUi(this);
    /// 设置初始大小
    this->resize(400,160);
...
//    this->showTitleLogoText( sDialogTitle,12,QChar(0xf069),10 );
    ...
    if (QFile::exists(sEbcIniFileSetting)) {
        ...
        GetPrivateProfileStringABoost("system","server","",lpszBuffer,sizeof(lpszBuffer),sEbcIniFileSetting.toStdString().c_str());
        m_oldServer = QString::fromUtf8(lpszBuffer);
        ui->lineEditServerAddress->setText(m_oldServer);
    }
}

DialogConnectSetting::~DialogConnectSetting()
{
    delete ui;
}

void DialogConnectSetting::accept(void)
{
