...
public:
#ifdef _QT_MAKE_
    void SetEventParameter(long p) {m_nEventParameter = p;}
    long GetEventParameter(void) const {return m_nEventParameter;}
    void SetEventBigParameter(mycp::bigint p) {m_nEventBigParameter = p;}
    mycp::bigint GetEventBigParameter(void) const {return m_nEventBigParameter;}
    void SetEventData(void* d) {m_pEventData = d;}
    void* GetEventData(void) const {return m_pEventData;}
    void SetEventString(const std::string& s) {m_sEventString = s;}
    const std::string& GetEventString(void) const {return m_sEventString;}
    void SetQEventType(Type e) {this->t = (ushort)e;}
    void setReceiver(QObject* receiver,unsigned long receiveKey) {
        m_receiver = receiver;
        m_receiveKey = receiveKey;
    }
    QObject* receiver(void) const {return m_receiver;}
    unsigned long receiveKey(void) const {return m_receiveKey;}
    void setReceiveResult(unsigned long receiveKey, long result) {m_receiveKey=receiveKey,m_nEventParameter = result;}
#endif

    const EB_Event& operator=(const EB_Event& p) {return this->operator =(&p);}
    const EB_Event& operator=(const EB_Event* p)
    {
        if (p!=NULL)
        {
#ifdef _QT_MAKE_
            this->m_nEventParameter = p->GetEventParameter();
            this->m_nEventBigParameter = p->GetEventBigParameter();
            this->m_pEventData = p->GetEventData();
            this->m_sEventString = p->GetEventString();
            this->t = p->type();
            this->m_receiver = p->receiver();
            this->m_receiveKey = p->receiveKey();
#endif
        }
        return *this;
    }

    EB_Event(const EB_Event& e)
#ifdef _QT_MAKE_
        : QEvent(e.type())
        , m_nEventParameter(e.GetEventParameter()), m_nEventBigParameter(e.GetEventBigParameter())
        , m_pEventData(e.GetEventData()), m_sEventString(e.GetEventString())
        , m_receiver(e.receiver()), m_receiveKey(e.receiveKey())
    #endif
    {
    }
#ifdef _QT_MAKE_
    EB_Event(Type e=QEvent::User)
        : QEvent(e)
        , m_nEventParameter(0), m_nEventBigParameter(0)
        , m_pEventData(0)
...
#ifdef _QT_MAKE_
private:
    long m_nEventParameter;
    mycp::bigint m_nEventBigParameter;
    void* m_pEventData;
    std::string m_sEventString;
    QObject* m_receiver;
    unsigned long m_receiveKey;
#endif
};
...
public:
	...
	{
		EB_Event::operator =(&pVersionInfo);
...
public:
	...
	const EB_EnterpriseInfo& operator =(const EB_EnterpriseInfo& pEnterpriseInfo)
	{
		EB_Event::operator =(&pEnterpriseInfo);
...
public:
	EB_GroupInfo(void)
		: EB_Event()
		, m_sEnterpriseCode(0)
		, m_sGroupCode(0), m_sParentCode(0)
	...
	EB_GroupInfo(eb::bigint nEnterpriseId, eb::bigint nGroupId)
		: EB_Event()
		, m_sEnterpriseCode(nEnterpriseId)
		, m_sGroupCode(nGroupId), m_sParentCode(0)
		, m_nCreateUserId(0),m_nManagerUserId(0),m_nDisplayIndex(0),m_nGroupType(EB_GROUP_TYPE_DEPARTMENT)
		, m_nMyEmpId(0),m_nEmpCount(0),m_nGroupVer(0)
		, m_nGroupExtData(0)
	{}
...
用户分组信息
===========================================================*/
class EB_UGInfo : public EB_Event
{
public:
	EB_UGInfo(void)
		: EB_Event(), m_nUGId(0)
	...
	EB_UGInfo(eb::bigint nUGId,const CEBString& sGroupName)
		: EB_Event(), m_nUGId(nUGId),m_sGroupName(sGroupName)
	{}
...
会话远程桌面信息
===========================================================*/
class EB_RemoteDesktopInfo : public EB_Event
{
public:
	EB_RemoteDesktopInfo(void)
		: EB_Event(), m_sCallId(0), m_nRDType(EB_RD_DESKTOP_SRC), m_nFromUserId(0), m_bOnDesktop(false)
	{
	}
...
会话视频信息
===========================================================*/
class EB_VideoInfo : public EB_Event
{
public:
	EB_VideoInfo(void)
		: EB_Event(), m_sCallId(0), m_nVideoType(EB_VIDEO_BOTH)
	{
	}
...
用户视频信息
===========================================================*/
class EB_UserVideoInfo : public EB_Event
{
public:
	EB_UserVideoInfo(void)
		: EB_Event(), m_sUserAccount(0), m_bOnVideo(false)//, m_nUserVideoId(0)
	{
	}
	EB_UserVideoInfo(eb::bigint sUser, bool bOnVideo)
		: EB_Event(), m_sUserAccount(sUser),m_bOnVideo(bOnVideo)//, m_nUserVideoId(nId)
	{}
...
帐号信息
===========================================================*/
class EB_AccountInfo : public EB_Event
{
public:
	EB_AccountInfo(void)
		: EB_Event(), m_nUserId(0),m_nPhone(0),m_nLogonType(0)
		, m_nGender(EB_GENDER_UNKNOWN)
		, m_nBirthday(0)
	...
	EB_AccountInfo(eb::bigint nUserId, const CEBString& sAccount,int nLogonType)
		: EB_Event(), m_nUserId(nUserId),m_sAccount(sAccount)
		, m_nPhone(0)
	...
	const EB_AccountInfo& operator =(const EB_AccountInfo* pAccountInfo)
	{
		...
		{
			EB_Event::operator =(pAccountInfo);
...
呼叫（会话）信息
===========================================================*/
class EB_CallInfo : public EB_Event
{
public:
	EB_CallInfo(void)
		: EB_Event(), m_sGroupCode(0),m_sOldCallId(0),m_sCallId(0),m_nFromUserId(0)
	{
	}
	EB_CallInfo(eb::bigint nFromUserId, const CEBString & sFromAccount, eb::bigint sCallId,eb::bigint sGroupCode)
		: EB_Event(), m_nFromUserId(nFromUserId),m_sFromAccount(sFromAccount),m_sCallId(sCallId),m_sGroupCode(sGroupCode),m_sOldCallId(0)
		//, m_nStateCode(0)
		//, m_bOwner(false)
	{
	}
