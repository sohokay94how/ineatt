...
typedef enum EB_RESOURCE_FROM_TYPE {
    ...
    , EB_RESOURCE_FROM_TYPE_ATT_REQ					// 考勤审批
...
typedef enum EB_STATE_CODE {
    EB_STATE_OK							= 0
    , EB_STATE_ERROR					= 1
    , EB_STATE_NOT_AUTH_ERROR					// 2 没有权限
    , EB_STATE_ACC_PWD_ERROR					// 3 帐号或密码错误
    , EB_STATE_NEED_RESEND						// 4 需要重发数据
    , EB_STATE_TIMEOUT_ERROR					// 5 超时错误
    , EB_STATE_EXIST_OFFLINE_MSG			// 6 存在离线消息
    , EB_STATE_USER_OFFLINE						// 7 用户离线状况
    , EB_STATE_USER_BUSY							// 8 用户线路忙
    , EB_STATE_USER_HANGUP						// 9 用户挂断会话
    , EB_STATE_OAUTH_FORWARD					// 10 OAUTH转发
    , EB_STATE_UNAUTH_ERROR						// 11 未验证错误
    , EB_STATE_ACCOUNT_FREEZE					// 12 帐号已经冻结
    , EB_STATE_NEED_DELETE_MSG				// 13 需要删除消息
    , EB_STATE_WAITING_PROCESS				// 14 等待处理（消息）
    , EB_STATE_PARAMETER_ERROR				// 15 参数错误
    , EB_STATE_DATABASE_ERROR					// 16 数据库操作错误
    , EB_STATE_NEW_VERSION						// 17 新版本
    , EB_STATE_FILE_ALREADY_EXIST			// 18 文件已经存在
    , EB_STATE_FILE_BIG_LONG					// 19 文件超过大小
    , EB_STATE_ACCOUNT_NOT_EXIST			// 20 帐号不存在
    , EB_STATE_ACCOUNT_ALREADY_EXIST		// 21 帐号已经存在
    , EB_STATE_ACCOUNT_DISABLE_OFFCALL	// 22 禁止离线会话
    , EB_STATE_ACCOUNT_DISABLE_EXTCALL	// 23 禁止外部会话
    , EB_STATE_DISABLE_REGISTER_USER	= 25		// 25 禁止用户注册功能
    , EB_STATE_DISABLE_REGISTER_ENT						// 26 禁止企业注册功能
    , EB_STATE_ENTERPRISE_ALREADY_EXIST	= 30	// 30 公司名称已经存在
    , EB_STATE_ENTERPRISE_NOT_EXIST						// 31 没有公司信息（企业不存在）
    , EB_STATE_DEP_NOT_EXIST									// 32 不存在群组（部门）
    , EB_STATE_EXIST_SUB_DEPARTMENT						// 33 存在子部门
    , EB_STATE_DEP_ACC_ERROR									// 34 群组或成员不存在
    , EB_STATE_ENT_ACC_ERROR							// 35 企业员工成员不存在
    , EB_STATE_CS_MAX_ERROR								// 36 超过客服座席最大数量
    , EB_STATE_NOT_CS_ERROR								// 37 没有客服座席
    , EB_STATE_EXCESS_QUOTA_ERROR					// 38 超过最大流量配额
    , EB_STATE_ENT_GROUP_ERROR						// 39 企业部门
    , EB_STATE_ONLINE_KEY_ERROR			= 40                    // 40
    , EB_STATE_UM_KEY_ERROR						// 41
    , EB_STATE_CM_KEY_ERROR						// 42
    , EB_STATE_DEVID_KEY_ERROR						// 43
    , EB_STATE_APPID_KEY_ERROR						// 44
    , EB_STATE_DEVID_NOT_EXIST						// 45
    , EB_STATE_APPID_NOT_EXIST						// 46
    , EB_STATE_APP_ONLINE_KEY_TIMEOUT                                   // 47
    , EB_STATE_KEY_ERROR						// 48 KEY错误
    , EB_STATE_ID_ERROR							// 49 ID错误
    ...
    , EB_STATE_PROCESS_UNFINISHED				// 90 （前一个）进程/作业未完成
    , EB_STATE_PROCESS_LINE_UP					// 91 进程/作业正在排队执行中

    , EB_STATE_PLAN_NOT_EXIST			= 100		// 100 计划不存在
    , EB_STATE_TASK_NOT_EXIST					// 101 任务不存在
    , EB_STATE_REPORT_NOT_EXIST					// 102 报告不存在
    , EB_STATE_ATTEND_REQ_NOT_EXIST				// 103 考勤申请不存在
